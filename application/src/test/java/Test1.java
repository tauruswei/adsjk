...
//        RedisTemplate redisTemplate = new RedisTemplate();
        String SELL_SCRIP2 = "local exists = redis.call ('EXISTS',KEYS[1])\n" +
                "if exists == 0 then\n" +
                "\treturn '\"ERROR\"'\n" +
                "else\n" +
        ...
        List<Boolean> results1 = (List<Boolean>) redisTemplate.execute((RedisCallback<List<Boolean>>) connection -> {
            return connection.scriptingCommands().scriptExists(redisScript.getSha1());
        });


        for (int i = 0; i < 3; i++) {
            redisTemplate.opsForSet().add(collectionKey,String.format("%d",i));
        }

        for (int i = 0; i < 3; i++) {
            log.info("循环 {}, 开始时间 {}",i,System.currentTimeMillis()/1000);

            List<Boolean> results2 = (List<Boolean>) redisTemplate.execute((RedisCallback<List<Boolean>>) connection -> {
                return connection.scriptingCommands().scriptExists(redisScript.getSha1());
            });
//            DefaultRedisScript<String> redisScript =  new DefaultRedisScript<>();
//            redisScript.setScriptText(SELL_SCRIP2);
//            redisScript.setResultType(String.class);
            String result = (String) redisTemplate.execute(redisScript, Collections.singletonList(collectionKey), new ArrayList<>());
            List<Boolean> results3 = (List<Boolean>) redisTemplate.execute((RedisCallback<List<Boolean>>) connection -> {
                return connection.scriptingCommands().scriptExists(redisScript.getSha1());
            });

            log.info("循环 {}, 结束时间 {}",i,System.currentTimeMillis()/1000);
        }
    }

    private static <T>DefaultRedisScript<T> initDefaultRedisScript(String script, Class<T> clazz){
